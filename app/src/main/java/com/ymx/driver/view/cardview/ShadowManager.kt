...
import com.ymx.driver.view.cardview.LCardView

class ShadowManager(colors: IntArray, percent: Float = 0.33f) {
    companion object {
    ...
    private val shaderArray = arrayOf(ltShader, tShadow, rtShader, rShadow, rbShader, bShadow, lbShader, lShadow)
    ...
    fun measureShadowPath(path: Path) {
        ...
        path.arcTo(RectF(lShadow.frame.right, tShadow.frame.bottom, lShadow.frame.right + ltShader.cornerRadius * 2, tShadow.frame.bottom + ltShader.cornerRadius * 2), 180f, 90f)
        path.lineTo(tShadow.frame.right, tShadow.frame.bottom)
        path.arcTo(RectF(rShadow.frame.left - rtShader.cornerRadius * 2, rShadow.frame.top - rtShader.cornerRadius, rShadow.frame.left, rShadow.frame.top + rtShader.cornerRadius), 270f, 90f)
        path.lineTo(rShadow.frame.left, rShadow.frame.bottom)
        path.arcTo(RectF(rShadow.frame.left - rbShader.cornerRadius * 2, rShadow.frame.bottom - rbShader.cornerRadius, rShadow.frame.left, rShadow.frame.bottom + rbShader.cornerRadius), 0f, 90f)
        path.lineTo(bShadow.frame.left, bShadow.frame.top)
        path.arcTo(RectF(lShadow.frame.right, lShadow.frame.bottom - lbShader.cornerRadius, lShadow.frame.right + lbShader.cornerRadius * 2, lShadow.frame.bottom + lbShader.cornerRadius), 90f, 90f)
        path.close()
    }

    fun measureContentPath(cardView: LCardView, paperSyncCorner: Boolean, paperCorner: Int, path: Path) {
        ...
        if (paperSyncCorner) {
            ...
            path.arcTo(RectF(startX, startY, ltShader.cornerRadius * 2 + startX, startY + ltShader.cornerRadius * 2), 180f, 90f)
            path.lineTo(viewWidth - stopX - rtShader.cornerRadius, startY)
            path.arcTo(RectF(viewWidth - stopX - rtShader.cornerRadius * 2, startY, viewWidth - stopX, startY + rtShader.cornerRadius * 2), 270f, 90f)
            path.lineTo(viewWidth - stopX, viewHeight - stopY - rbShader.cornerRadius)
            path.arcTo(RectF(viewWidth - stopX - rbShader.cornerRadius * 2, viewHeight - stopY - rbShader.cornerRadius * 2, viewWidth - stopX, viewHeight - stopY), 0f, 90f)
            path.lineTo(startX + lbShader.cornerRadius, viewHeight - stopY)
            path.arcTo(RectF(startX, viewHeight - stopY - lbShader.cornerRadius * 2, startX + lbShader.cornerRadius * 2, viewHeight - stopY), 90f, 90f)
        } else {
            if (paperCorner == 0) {
    ...
    fun createDrawables(cardView: LCardView, shadowSize: Int) {
        ...
        right = if (paddingRight > 0) viewWidth.toFloat() else viewWidth + effectRightOffset.toFloat()
        left = right - shadowSize
        top = if (paddingTop > 0) rtRadius.toFloat() else rtRadius - effectTopOffset.toFloat()
        bottom = if (paddingBottom > 0) (viewHeight - rbRadius).toFloat() else viewHeight - rbRadius + effectBottomOffset.toFloat()
        ...
        right = if (paddingRight > 0) (viewWidth - rbRadius).toFloat() else viewWidth - rbRadius + effectRightOffset.toFloat()
        top = if (paddingBottom > 0) (viewHeight - shadowSize).toFloat() else viewHeight - shadowSize + effectBottomOffset.toFloat()
        bottom = top + shadowSize
        bShadow.frame[left, top, right] = bottom
        bShadow.onFrameChange()
        right = if (paddingLeft > 0) shadowSize.toFloat() else shadowSize.toFloat() - effectLeftOffset
        left = right - shadowSize
        top = if (paddingTop > 0) ltRadius.toFloat() else ltRadius - effectTopOffset.toFloat()
        bottom = if (paddingBottom > 0) (viewHeight - lbRadius).toFloat() else viewHeight - lbRadius + effectBottomOffset.toFloat()
