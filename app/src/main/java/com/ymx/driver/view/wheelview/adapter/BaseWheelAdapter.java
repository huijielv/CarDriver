 ...
 */
package com.ymx.driver.view.wheelview.adapter;
...
import com.ymx.driver.view.wheelview.widget.IWheelView;
...
public abstract class BaseWheelAdapter<T> extends BaseAdapter {
    ...
    public boolean isEnabled(int position) {
        if (mClickable) {
            if (mLoop) {
                return position % mList.size() == mCurrentPositon;
            } else {
                return position == (mCurrentPositon + mWheelSize / 2);
            }
        }
        return false;
    }
