...
public class ClassicPopupWindow extends PopupWindow {
    ...
    private boolean isNeedShowAsTop(View anchor) {
        ...
        int x = location[0];
        int y = location[1];
        int yNew = y - UIUtils.getStatusBarHeight();
        //
        int contentViewHeight = getContentViewHeight();
        //
...
//        }
        ...
        if (Build.VERSION.SDK_INT == 24 || Build.VERSION.SDK_INT == 25 || Build.VERSION.SDK_INT == 26 || Build.VERSION.SDK_INT == 27 || Build.VERSION.SDK_INT == 28 || Build.VERSION.SDK_INT == 29) {
            int[] location = new int[2];
