...
public class FileUtil {
    ...
    private static final String SAMPLE_DIR = "HuoLi/TTS";

    // 创建一个临时目录，用于复制临时文件，如assets目录下的离线资源文件
    public static String createTmpDir(Context context) {

        String tmpDir = Environment.getExternalStorageDirectory().toString() + "/" + SAMPLE_DIR;
        if (!FileUtil.makeDir(tmpDir)) {
            tmpDir = context.getExternalFilesDir(SAMPLE_DIR).getAbsolutePath();
            if (!FileUtil.makeDir(SAMPLE_DIR)) {
    ...
    public static String createTmpDir(Context context, String path) {
        ...
        String tmpDir = Environment.getExternalStorageDirectory().toString() + "/" + path;
        if (!FileUtil.makeDir(tmpDir)) {
